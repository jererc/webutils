...
class ContextTestCase(unittest.TestCase):
    ...
    def test_1(self):
        with module.playwright_context(module.State(self.state_file), headless=False) as context:
            page = context.new_page()
            page.goto('https://www.google.com')
            module.save_page(page, self.save_dir, 'test', save_content=True, save_screenshot=True)
        self.assertTrue(os.path.exists(self.state_file))
        save_files = glob(os.path.join(self.save_dir, f'{module.PAGE_FILE_PREFIX}-*'))
        pprint(save_files)
